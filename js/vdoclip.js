...
function Connect_DB() {
  ...
  dbAPEclip = firebase.firestore().collection("APEclip");
  CheckProfile();
}
...
function CheckProfile() {
  ...
  var str = "";
  var checkVDO = 0;
  dbAPEclip.where('LineID','==',sessionStorage.getItem("LineID"))
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      EidSongkarn = doc.id;
      sessionStorage.setItem("UploadSongkarn", doc.data().SendImg);
      document.getElementById("txtDetail").value = doc.data().SendMemo;
      //alert(doc.data().ClipStatus);
      if(doc.data().ClipStatus==1) {
        //if(doc.data().SendImg!="") {
          checkVDO = 1;
          str += '<video id="video" style="max-width:450px;width:90%;" controls="controls" autoplay>';
          str += '<source src="'+doc.data().SendImg+'" type="video/mp4"></video></div>';
          $("#outputImg").html(str);
        //}
      }
...
function ClickSongkarn() {
  ...
  if(sCheckBottom==3) {
    ...
    alert("คุณยังขาดข้อมูลที่ต้องกรอกอีก = "+(3-sCheckBottom)+" รายการ");
  }
}
...
function SaveSongkarn() {
  ...
  if(EidSongkarn=="") {
    ...
    dbAPEclip.doc(EidSongkarn).update({
      SendMemo : document.getElementById("txtDetail").value,
      SendUnderRH : document.getElementById("txtUnderRH").value,
      SendAddress : document.getElementById("txtAddress").value,
      Senddate : dateString
