...
var dbStockList = firebase.firestore().collection("APEclip");
var dateString="";
var x = 0;
...
function loadData(id) {
  ...
  dbStockList.where('ClipStatus','==',parseInt(id))
...
function ClickID(x,id) {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      EidStockList = doc.id;
      str1 += '<video id="video" style="max-width:450px;width:90%;" controls="controls" autoplay>';
      str1 += '<source src="'+doc.data().SendImg+'" type="video/mp4"></video></div>';

      str += '<div style="margin-top:10px;">';
      str += '<div class="redeem-header">'+ doc.data().EmpName +' ('+doc.data().EmpID+') สังกัด '+doc.data().SendUnderRH+'</div>';
      str += '<div style="width:500px;margin:auto;text-align: center">'+ str1 +'</div>';

      str += '<div style="width:100%;max-width:600px;margin:auto;">';
      str += '<div class="txt-form" style="margin-top:15px;text-align:left;font-size:11px;">URL รูปภาพ (ขนาดก้วาง 420px สูง 300px)</div>';
      str += '<input type="text" class="form-control" id="txtDisplayImg" value="'+doc.data().DisplayImg+'" style="font-size:11px;" placeholder="รูปภาพประกอบ ..."/>';

      str += '<div class="txt-form" style="margin-top:15px;text-align:left;font-size:11px;">URL คลิปวิดีโอ</div>';
      str += '<input type="text" class="form-control" id="txtSendImg" value="'+doc.data().SendImg+'" style="height:50px;font-size:11px;" placeholder="คลิปวิดีโอ ..."/>';

      str += '<div class="txt-form" style="margin-top:15px;text-align:left;font-size:11px;">สถานะ Online (0=OFFline, 1=Online)</div>';
      str += '<input type="text" class="form-control" id="txtClipStatus" value="'+doc.data().ClipStatus+'" style="width:60px;font-size:11px;" placeholder="สถานะ Online ..."/>';
      str += '</div>';
      ...
      str += '<div class="btn-t1" onclick="SaveData()">บันทึกรายการ</div>';
...
function SaveData() {
  var stxtDisplayImg = document.getElementById("txtDisplayImg").value;
  var stxtSendImg = document.getElementById("txtSendImg").value;
  var stxtClipStatus = document.getElementById("txtClipStatus").value;
  //alert(stxtDisplayImg+"\n"+stxtSendImg+"\n"+stxtClipStatus+"\n"+EidStockList);
  dbStockList.doc(EidStockList).update({
    SendImg : stxtSendImg,
    DisplayImg : stxtDisplayImg,
    ClipStatus : parseFloat(stxtClipStatus)
  });
  //alert("Save Data");
